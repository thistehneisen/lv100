...
// - - - - - - - - - - -  Calendar month fake month switch  - - - - - - - - - - -
	...
	$(document).on('click', ".calendar-month .header .right", function(e) {
		if ($('.month.new').next('.month').length == 0) {
			...
			$('.calendar-month .header h2').html($(".month.new").data("monthvisible"));
			if ($('.month.new').nextAll('.month').length == 0) {
				$.get(location.href, {month: $(".month.new").data("month")}, function(response) {
					$(".month.new").after($(response).find(".month.new").next(".month"));
					$('.calendar-month .header .disabled').removeClass('disabled');
				});
			}
		}
	...
	$(document).on('click', ".calendar-month .header .left", function(e) {
		if ($('.month.new').prev('.month').length == 0) {
			...
			$('.calendar-month .header .disabled').removeClass('disabled');
			$('.calendar-month .header h2').html($(".month.new").data("monthvisible"));
			if ($('.month.new').prevAll('.month').length == 0) {
				$.get(location.href, {month: $(".month.new").data("month")}, function(response) {
					$(".month.new").before($(response).find(".month.new").prev(".month"));
					$('.calendar-month .header .disabled').removeClass('disabled');
				});
			}
		}
	...
	$('#calendar .header h2').html($(".month.new").data("monthvisible"));
